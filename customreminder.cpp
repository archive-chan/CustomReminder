...
void CustomReminder::initUi()
{
	...
	homeAction = ui->toolBar->addAction("主页");
	presetsAction = ui->toolBar->addAction("预设");
	settingsAction = ui->toolBar->addAction("设置");
	recoverAction = m_trayMenu->addAction("还原窗口");
	exitAction = m_trayMenu->addAction("退出");

	ui->toolBar->setMovable(false);
	ui->stackedWidget->setCurrentIndex(0);
	ui->hSpinBox->setMaximum(23);
	ui->mSpinBox->setMaximum(59);
	ui->sSpinBox->setMaximum(59);
	ui->hSpinBox->setWrapping(true);
	ui->mSpinBox->setWrapping(true);
	ui->sSpinBox->setWrapping(true);
	...
	WinToast::instance()->setAppName(L"自定义提醒");
	WinToast::instance()->setAppUserModelId(WinToast::configureAUMI(L"ARCHIVE", L"CUSTOMREMINDER", L"CustomReminder", L"240212Ver1_2"));
	...
	connect(ui->hSpinBox, &QSpinBox::textChanged, this, &CustomReminder::changeSlot);
	connect(ui->mSpinBox, &QSpinBox::textChanged, this, &CustomReminder::changeSlot);
	connect(ui->sSpinBox, &QSpinBox::textChanged, this, &CustomReminder::changeSlot);
	...
	connect(ui->RepeatTimeComboBox, &QComboBox::textActivated, this, &CustomReminder::changeSlot);
...
void CustomReminder::updateUi()
{
	...
	ui->mSpinBox->blockSignals(true);
	ui->sSpinBox->blockSignals(true);
	...
	ui->RepeatTimeComboBox->addItem("0");
	ui->RepeatTimeComboBox->addItem("5");
	ui->RepeatTimeComboBox->addItem("10");
	ui->RepeatTimeComboBox->addItem("15");
	ui->RepeatTimeComboBox->addItem("30");
	ui->RepeatTimeComboBox->addItem("60");
	...
	ui->hSpinBox->setValue(m_configManager->getValue(m_presetToUpdate, "时").toInt());
	ui->mSpinBox->setValue(m_configManager->getValue(m_presetToUpdate, "分").toInt());
	ui->sSpinBox->setValue(m_configManager->getValue(m_presetToUpdate, "秒").toInt());
	...
	if (ui->RepeatComboBox->currentText() == "始终") {
		ui->RepeatTimeComboBox->setCurrentText(m_configManager->getValue(m_presetToUpdate, "间隔时间"));
	}
	else {
		ui->RepeatMinLabel->hide();
		ui->RepeatTimeComboBox->hide();
	}

	ui->hSpinBox->blockSignals(false);
	ui->mSpinBox->blockSignals(false);
	ui->sSpinBox->blockSignals(false);
}
...
void CustomReminder::selectPreset(const QString& preset)
{
	...
	ui->hSpinBox->setValue(m_configManager->getValue(preset, "时").toInt());
	ui->mSpinBox->setValue(m_configManager->getValue(preset, "分").toInt());
	ui->sSpinBox->setValue(m_configManager->getValue(preset, "秒").toInt());
...
void CustomReminder::startButtonSlot()
{
	...
	if (ui->RepeatComboBox->currentText() == "始终") {
		//始终重复
		QString minute = ui->RepeatTimeComboBox->currentText();
...
void CustomReminder::newButtonSlot()
{
	...
	ui->RepeatTimeComboBox->setCurrentText("5");
...
void CustomReminder::saveButtonSlot()
{
	if (ui->PresetsComboBox->isEditable()) {
		...
		if (ui->hSpinBox->value() == 0 && ui->mSpinBox->value() == 0 && ui->sSpinBox->value() == 0) {
	...
	m_configManager->setValue(m_presetToUpdate, "时", ui->hSpinBox->text());
	m_configManager->setValue(m_presetToUpdate, "分", ui->mSpinBox->text());
	m_configManager->setValue(m_presetToUpdate, "秒", ui->sSpinBox->text());
	...
	if (ui->RepeatComboBox->currentText() == "始终") {
		m_configManager->setValue(m_presetToUpdate, "间隔时间", ui->RepeatTimeComboBox->currentText());
	}
	m_configManager->saveConfig();
...
void CustomReminder::timerOutSlot()
{
	...
	if (msec > 0) {
		...
		if (ui->RepeatComboBox->currentText() == "仅一次") {
			...
			if (ui->RepeatTimeComboBox->currentText().toInt() == 0) {
				ui->TimerLabel->setText(m_time.toString("hh:mm:ss"));
			}
			else {
				m_timer->stop();
...
void CustomReminder::timerMiddleOutSlot()
{
	...
	if (msec > 0) {
		...
		showNotification("时间到了", ui->RepeatTimeComboBox->currentText() + "分钟过去了~");
...
{
	if (ui->TrayCheckBox->checkState() == Qt::Checked) {
		this->hide();
